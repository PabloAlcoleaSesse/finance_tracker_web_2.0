            ...
            <section className="relative grid h-full w-full overflow-hidden bg-black/50 backdrop-blur-sm lg:grid-cols-2">
                <aside className="hero-gradient-card hidden h-full flex-col justify-between p-12 text-white lg:flex">
                    ...
                    <p className="max-w-sm text-sm font-medium leading-relaxed text-white/80">
                        Track positions, net worth, and performance with the same clear visual language as your landing workspace.
                    </p>
                </aside>
                ...
                <div className="flex items-center justify-center bg-black/65 p-8 text-white sm:p-10 lg:p-12">
                    <div className="mx-auto w-full max-w-md">
                        ...
                        {!needsTwoFactor ? (
                            <>
                                ...
                                <div className="my-5 flex items-center gap-3">
                                    ...
                                    <span className="text-[10px] uppercase tracking-[0.18em] text-white/50">or use email</span>
                                    <span className="h-px flex-1 bg-white/15" />
                                </div>
                            </>
                        ) : null}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {needsTwoFactor ? (
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        maxLength={6}
                                        value={twoFactorCode}
                                        onChange={(event) => setTwoFactorCode(normalizeCodeInput(event.target.value))}
                                        placeholder="123456"
                                        className="h-12 w-full rounded-xl border border-white/15 bg-black/55 px-4 text-white placeholder-white/45 outline-none ring-0 transition focus:border-[#FFB95D]/70 focus:shadow-[0_0_0_4px_rgba(255,185,93,0.18)]"
                                    />
                                    <p className="text-xs text-neutral-400">Code refreshes every 30s.</p>
                                </div>
                            ) : null}

                            <div className="flex items-center justify-between gap-4 pt-1">
                                <label className="inline-flex items-center gap-2 text-sm text-white/70">
                                    <input type="checkbox" className="h-4 w-4 rounded border-white/25 bg-transparent text-[#FFB95D] focus:ring-[#FFB95D]" />
                                    Remember me
                                </label>
                                <Link href="/auth/forgot-password" className="text-sm font-semibold text-white/70 transition hover:text-[#FFB95D]">
                                    Forgot password?
                                </Link>
                            </div>
                            ...
                            <button
                                type="submit"
                                disabled={isSubmitting || (needsTwoFactor && twoFactorCode.length !== 6)}
                            ...
                            >
                                {isSubmitting ? "Signing in..." : needsTwoFactor ? "Verify and Log In" : "Log In"}
                            </button>

                            <p className="pt-2 text-center text-sm text-white/70">
                                New here?{" "}
                                <Link href="/auth/register" className="font-semibold text-[#FFB95D] transition hover:text-[#ffd39a]">
                                    Create account
                                </Link>
                            </p>
                        </form>
