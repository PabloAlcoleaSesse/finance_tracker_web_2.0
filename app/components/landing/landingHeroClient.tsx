...
function HeroThreeGradient() {
  ...
  useEffect(() => {
    ...
    // Pause rendering when section is not visible to reduce main-thread work
    ...
    let rafId = 0;
    const tick = () => {
      // Only render when section is visible to reduce main-thread work
...
export default function LandingHeroClient({
  ...
  useEffect(() => {
    ...
    const words = Array.from(
      paragraph.querySelectorAll<HTMLElement>(".hero-word"),
    );
  ...
  return (
    ...
    >
      ...
      <p
        ref={paragraphRef}
        className="absolute bottom-[20%] z-20 max-w-[min(90vw,64rem)] px-4 text-2xl font-sans font-regular text-white sm:text-3xl md:text-4xl"
      >
